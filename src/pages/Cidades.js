...
class Cidades extends Component {
    constructor(props, context) {
        ...
        this.filterEstado = this.filterEstado.bind(this);
    }
    ...
    filterEstado(item){
        let newFilter = this.state.filter
        newFilter.fk_est = item.value
        newFilter.estado = item
        this.setState({ filter: newFilter })
    }
    ...
    async showModal(e, codigo) {
        ...
        if (Number(codigo) > 0) {
            edicao = true
            pk = codigo
            await fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/getCidades?pk='+(Number(codigo)).toString()).then(r => r.json()).then(async r => {
            // await fetch(config.backend+'/getCelulares?pk='+(Number(e.target.id)).toString()).then(r => r.json()).then(async r => {
                if (typeof r[0] === 'undefined') {
                    ...
                    let form = document.getElementById('registroCidades');
                    let selectedPais = this.state.combos.filter((item) => { return item.type==='paises' && item.value===r[0].fk_pai })
                    let selectedEstado = this.state.combos.filter((item) => { return item.type==='estados' && item.value===r[0].fk_est })
                    this.setState({ selected: {
                        pais: selectedPais[0],
                        estado: selectedEstado[0]
                    }})
            ...
            edicao = false
            document.getElementById("registroCidades").reset();
        }
    ...
    submitData(e) {
        ...
        const form = document.getElementById('registroCidades');
        ...
        if (this.state.edit) {
            //Editar
            console.log(json)
            fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/editCidades?pk='+this.state.codigo, {
                method: 'POST',
                headers: {
            ...
            //Inserir
            fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/novoCidades', {
                method: 'POST',
                headers: {
                    'Content-type': 'application/json'
                },
    ...
    async componentDidMount() {
        ...
        //Estados
        await fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/getEstados').then(r => r.json()).then(async r => {
            let combosEstados = this.state.combos
            await asyncForEach(r, async (item)=>{
               let estados = {
                    type: 'estados',
                    display: item.nome,
                ...
                await combosEstados.push(estados)
            })
            this.setState({combos: this.state.combos})
        }) 
        ...
        query.fk_est = Number(getParameterByName('fk_est'))
        query.estados = this.state.combos.filter((item) => { return item.type==='estados' && Number(item.value)===query.fk_est})[0]
    ...
    handleDelete(e, pk) {
        ...
        swal({
            ...
            if (result) {
                //Delete
                fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/deleteCidades?pk='+pk, {
                    method: 'POST',
                    headers: {
                        'Content-type': 'application/json'
                    },
    ...
    handleClean(e) {
        ...
        e.preventDefault()
        console.log('limpa')
        window.history.replaceState({filtered: false}, 'filter', "/cidades") //Apaga QueryURL
    ...
    async filterData(data) {
        ...
        return new Promise(async (resolve)=>{
            ...
            let filtered = await data.filter((item) => {
                ...
                //Filtro: Lotação
                let fk_est = (Number(item.fk_est) === Number(filter.fk_est)) || (filter.fk_est || '') === ''
                if (filter.fk_est) {
                    if (queryString === '?') {
                        queryString = queryString + 'fk_est=' + filter.fk_est
                    } else queryString = queryString + '&fk_est=' + filter.fk_est
                }

                //Monta Query URL
                if (queryString !== '?') {
                    window.history.replaceState({filtered: true}, 'filter', "/cidades"+queryString+"&filtered=true")
                } else {
                    window.history.replaceState({filtered: true}, 'filter', "/cidades")                
                }

                //Filtra
                return nome && sigla && fk_pai && fk_est
            })
            resolve(filtered)
        })        
    }
    ...
    getData(e) {
        ...
        e.preventDefault()
        //Busca
        fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/getCidades').then(r => r.json()).then(async r => {
            //Filtra
    ...
    render() {
        ...
        let estados = this.state.combos.filter((item) => { return item.type==='estados' })
        return (
            <div className="boxSite colorSettings">
                {/***************** Barra de Navegação *******************/}
                <div className="boxNavBar">
                    <NavBar selected="Cidades"></NavBar>
                </div>
                {/***************** Tela do WebSite *******************/}
                <div className="boxTela">
                    {/*********************** Header ***********************/}
                    <div className="boxHeader">
                        <h3 className="headerCadastro">Cadastro de Cidades</h3>
                    </div>
                    {/*********************** Filtros ***********************/}
                    <div className="boxFiltros">
                        ...
                        <div className="filtros">
                            <div className="column-filter">
                                ...
                                <div className='itemFiltro'>
                                    <label className="labelFiltro">Estado</label>
                                    <Autocomplete field="filtroFk_est" items={estados} inModal="" selected={this.state.filter.estado || { display: '' }} select={this.filterEstado}></Autocomplete>
                                </div>
                            </div>
                            <br/>
                            ...
                            <Modal.Dialog className="Modal">
                                    ...
                                    <Modal.Header className="ModalBg">   
                                        <div className="ModalHeader">
                                            <h3 className="headerModal">Registro de Cidade</h3>
                                        </div>
                                    </Modal.Header>
                                    <Modal.Body className="ModalBg" >   
                                        <div className='ModalBody'> 
                                            <form id="registroCidades" name="registroCidades" onSubmit={ this.submitData }>
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <Autocomplete field="fk_est" items={estados} inModal="-inModal" selected={this.state.selected.estado} select={this.selectEstado}></Autocomplete>
                                                </div>
                                            </form>
                                        </div>
                                    </Modal.Body>
                                        ...
                                        },
                                        {
                                            ...
                                            Cell: row => { return (
                                                <div className="buttonsDetailColumn">
                                                    <button className="buttonDetailColumn" onClick={(e)=>{this.showModal(e, row.row.pk_cid)}}>
                                                        <Icon size={20} icon={edit}></Icon>
                                                        Editar
                                                    </button>
                                                    <button className="buttonDetailColumn" onClick={(e)=>{this.handleDelete(e, row.row.pk_cid)}}>
