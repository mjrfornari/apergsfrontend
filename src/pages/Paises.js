...
class Paises extends Component {
    ...
    async showModal(e, codigo) {
        ...
        if (Number(codigo) > 0) {
            edicao = true
            pk = codigo
            await fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/getPaises?pk='+(Number(codigo)).toString()).then(r => r.json()).then(async r => {
            // await fetch(config.backend+'/getCelulares?pk='+(Number(e.target.id)).toString()).then(r => r.json()).then(async r => {
                if (typeof r[0] === 'undefined') {
                    ...
                    let form = document.getElementById('registroPaises');
    ...
    submitData(e) {
        ...
        const form = document.getElementById('registroPaises');
        ...
        if (this.state.edit) {
            //Editar
            console.log(json)
            fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/editPaises?pk='+this.state.codigo, {
                method: 'POST',
                headers: {
            ...
            //Inserir
            fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/novoPaises', {
                method: 'POST',
                headers: {
                    'Content-type': 'application/json'
                },
    ...
    handleDelete(e, pk) {
        ...
        swal({
            ...
            if (result) {
                //Delete
                fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/deletePaises?pk='+pk, {
                    method: 'POST',
                    headers: {
                        'Content-type': 'application/json'
                    },
    ...
    handleClean(e) {
        ...
        e.preventDefault()
        console.log('limpa')
        window.history.replaceState({filtered: false}, 'filter', "/paises") //Apaga QueryURL
    ...
    async filterData(data) {
        ...
        return new Promise(async (resolve)=>{
            ...
            let filtered = await data.filter((item) => {
                ...
                //Monta Query URL
                if (queryString !== '?') {
                    window.history.replaceState({filtered: true}, 'filter', "/paises"+queryString+"&filtered=true")
                } else {
                    window.history.replaceState({filtered: true}, 'filter', "/paises")                
                }
    ...
    getData(e) {
        ...
        e.preventDefault()
        //Busca
        fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/getPaises').then(r => r.json()).then(async r => {
            //Filtra
        ...
        return (
            <div className="boxSite colorSettings">
                ...
                <div className="boxNavBar">
                    <NavBar selected="Paises"></NavBar>
                </div>
                {/***************** Tela do WebSite *******************/}
                <div className="boxTela">
                    {/*********************** Header ***********************/}
                    <div className="boxHeader">
                        <h3 className="headerCadastro">Cadastro de Países</h3>
                    </div>
                    {/*********************** Filtros ***********************/}
                            ...
                            <Modal.Dialog className="Modal">
                                    ...
                                    <Modal.Header className="ModalBg">   
                                        <div className="ModalHeader">
                                            <h3 className="headerModal">Registro de País</h3>
                                        </div>
                                    </Modal.Header>
                                    <Modal.Body className="ModalBg" >   
                                        <div className='ModalBody'> 
                                            <form id="registroPaises" name="registroPaises" onSubmit={ this.submitData }>
                                                <div>
                                                    <label className="labelModal">Nome</label>
                                                    <input type="text" id="nome" name="nome" className="form-control" data-parse="uppercase" />
                                                </div>
                                            </form>
                                        </div>
                                    </Modal.Body>
                                        ...
                                        },
                                        {
                                            ...
                                            Cell: row => { return (
                                                <div className="buttonsDetailColumn">
                                                    <button className="buttonDetailColumn" onClick={(e)=>{this.showModal(e, row.row.pk_pai)}}>
                                                        <Icon size={20} icon={edit}></Icon>
                                                        Editar
                                                    </button>
                                                    <button className="buttonDetailColumn" onClick={(e)=>{this.handleDelete(e, row.row.pk_pai)}}>
