...
class RegistroAssociados extends Component {
    ...
    async showModal(e) {
        ...
        if (Number(e.target.id) > 0) {
            edicao = true
            pk = e.target.id
            await fetch(config.protocol+'://'+config.server+':'+config.portBackend+'/api/getCelulares?pk='+(Number(e.target.id)).toString()).then(r => r.json()).then(async r => {
                        ...
                        <Modal.Dialog className="Modal">
                                ...
                                <Modal.Header className="ModalBg">   
                                    <div className="ModalHeader">
                                        <h3 className="headerModal">Cadastro de Celulares</h3>
                                    </div>
                                </Modal.Header>
                                    ...
                                    }, 
                                    {
                                        ...
                                        Cell: row => { return (
                                            <div className="buttonsDetailColumn">
                                                <button className="buttonDetailColumn" id={row.row.pk_ace} name="editar" onClick={this.showModal}><Icon size={20} icon={edit}></Icon>Editar</button>
                                                <button className="buttonDetailColumn"><Icon size={20} icon={iosTrash}></Icon>Excluir</button>
                                            </div>
                                        )}
                                    }
                                ]}
